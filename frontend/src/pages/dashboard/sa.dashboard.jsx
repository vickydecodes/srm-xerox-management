...
export default function SaDashboard({ data, refreshData }) {
  ...
  const kpis = [
    ...
    },
    {
      ...
      description: "Active physical centers",
  ...
  const quickActions = [
    ...
    },
    {
      ...
      subtitle: "Configure center settings",
      icon: <IconBuildingCommunity className="w-5 h-5" />,
      path: "/super_admin/branch",
    },
    {
      title: "Register Operator",
      subtitle: "Add new administrators",
      icon: <IconUsers className="w-5 h-5" />,
      path: "/super_admin/department-admin",
    },
    {
      title: "Service Catalog",
      subtitle: "Configure pricing sheets",
      icon: <IconClipboardList className="w-5 h-5" />,
      path: "/super_admin/service",
    },
  ];
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border border-border shadow-sm bg-card">
            <CardHeader>
              ...
              <CardDescription>Paid sales trajectory.</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border border-border shadow-sm bg-card">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Latest checkout logs in the system.</CardDescription>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate("/super_admin/bill")}
            ...
            <CardContent className="p-0 overflow-x-auto">
              {bills.length === 0 ? (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-muted/40 border-y border-border text-[10px] text-muted-foreground font-bold uppercase tracking-wider text-center sm:text-left">
            ...
            <CardContent>
              {barData.length === 0 ? (
                <div className="h-44 flex items-center justify-center text-xs text-muted-foreground">
                  No branch sales metrics recorded.
                </div>
              ) : (
                <ChartContainer config={chartConfig} className="h-56 w-full">
