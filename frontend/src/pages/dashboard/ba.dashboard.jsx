...
export default function BaDashboard({ data, refreshData }) {
  ...
  const kpisMain = [
    ...
    },
    {
      ...
      description: "Waiting for branch approvals",
      icon: <IconClipboardList className="w-8 h-8 text-primary opacity-80" />,
    },
  ];

  const kpisMinor = [
    { title: "Active Departments", value: stats.departments, icon: <IconBuildingCommunity className="w-5 h-5 text-primary" /> },
    { title: "Branch Users", value: stats.users, icon: <IconUsers className="w-5 h-5 text-primary" /> },
    { title: "Inventory Products", value: stats.inventoryProducts || 0, icon: <IconReceipt className="w-5 h-5 text-primary" /> },
  ];
        ...
        <div className="lg:col-span-2 space-y-4 lg:space-y-6">
          ...
          <div className="flex flex-col bg-card border border-border/60 rounded-lg shadow-sm overflow-hidden">
            <div className="p-5 border-b border-border/60 flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => navigate("/branch_admin/bill")} className="text-xs h-7">
                View All
              </Button>
            </div>
