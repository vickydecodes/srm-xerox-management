...
const controllers = {
  createInventoryProduct: async (
    ...
    const inventoryProduct = await service.createInventoryProduct(req.body);
  ...
  getInventoryProductById: async (req: Request<{ id: string }>, res: Response) => {
    ...
    const inventoryProduct = await service.getInventoryProductById(id);

    if (!inventoryProduct) return sendResponse.notFound(res, 'inventory product');

    return sendResponse.fetched(res, 'inventory product', inventoryProduct);
  },
  ...
  updateInventoryProduct: async (
    ...
    const inventoryProduct = await service.updateInventoryProduct(id, req.body);

    if (!inventoryProduct) return sendResponse.notFound(res, 'inventory product');

    return sendResponse.updated(res, 'inventory product', inventoryProduct);
  },

  deleteInventoryProduct: inventoryProductStatus.softDelete,

  setInventoryProductActiveStatus: inventoryProductStatus.setActiveStatus,

  retrieveInventoryProduct: inventoryProductStatus.retrieve,

  eraseInventoryProduct: async (req: Request<{ id: string }>, res: Response) => {
    const { id } = req.params;

    const inventoryProduct = await service.eraseInventoryProduct(id);
